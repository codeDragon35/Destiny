  ...
  return (
    <main className="min-h-dvh bg-space">
      <div className="mx-auto max-w-3xl px-6 py-16 sm:px-10 sm:py-24">
        ...
        <h1 className="mt-10 text-4xl font-semibold text-ivory sm:text-5xl">My trips</h1>
        <p className="mt-3 text-sm text-soft-gray">{session.user.email}</p>

        {trips.length === 0 ? (
          <div className="mt-14 rounded-2xl border border-white/5 bg-midnight p-8 text-center">
          ...
          <ul className="mt-14 space-y-4">
            {trips.map((trip) => (
              <li key={trip.slug}>
                <Link
                  href={`/trip/${trip.slug}`}
                  className="group flex items-center justify-between gap-6 rounded-2xl border border-white/5 bg-midnight px-6 py-5 transition hover:border-jade/40"
                >
